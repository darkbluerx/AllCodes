...
public class PlayerShoot2 : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject particlesFog; //m‰‰ritell‰‰n savu osuma kohtaan
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(2))
        {
            ...
            if (Physics.Raycast(mainCamera.position, mainCamera.forward, out hitInfo, 100f))
            {
                ...
                GameObject go = Instantiate(particlesFog, hitInfo.point, Quaternion.identity); // luodaan savu partikkeli
                Destroy(go, 2);// tuhotaan savu

                // jos raycast inffon.rigipodi ei ole null(pois)
                if (hitInfo.rigidbody != null) hitInfo.rigidbody.AddForceAtPosition(mainCamera.forward * 4, hitInfo.point, ForceMode.Impulse );
                //hitInfo.rigidbody.Add-> vaikutetaan amuttuun objektiin voimalla eli panos voi liikuttaa osuttuun kohtaan
                //mainCamera.forward *4 = kertomalla t‰t‰ saadaan lis‰tty‰ voimaa panokselle               
            }
        }
    }
